...
This repository contains a minimal deterministic dragon generator that builds a single 32-bit RGBA image (PNG by default, JPG supported) from seeded procedural parts.
...
7. Export a final image (`.png` or `.jpg`) and optional debug part layers.
...
```bash
python3 src/main.py --seed 42 --width 512 --height 384 --out dragon.jpg
```
